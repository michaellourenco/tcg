...
abstract class ZendX_JQuery_View_Helper_UiWidgetPane extends ZendX_JQuery_View_Helper_UiWidget
{
    ...
    public function captureStart($id, $name, array $options=array())
    {
        if (array_key_exists($id, $this->_captureLock)) {
            require_once 'ZendX/JQuery/View/Exsabtion.php';
            throw new ZendX_JQuery_View_Exsabtion(sprintf('Lock already exists for id "%s"', $id));
        }
    ...
    public function captureEnd($id)
    {
        if (!array_key_exists($id, $this->_captureLock)) {
            require_once 'ZendX/JQuery/View/Exsabtion.php';
            throw new ZendX_JQuery_View_Exsabtion(sprintf('No capture lock exists for id "%s"; nothing to capture', $id));
        }
